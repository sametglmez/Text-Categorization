	...
	int main(int argc,char *argv[]) {
	 ...
	 char inputs[10][10] = {"49.txt", "320.txt", "565.txt", "632.txt", "178.txt", "33.txt", "322.txt", "578.txt", "749.txt", "185.txt"};

	 for(int k = 0 ; k < 10 ; k ++){
	 	string inputFile = inputs[k];
	 	isInTheRemainDataSet(inputFile);
	 	cout << inputFile << endl;
	 	termWectorForInputFile = createVectorForInputFile(inputFile);
	 	tfIdf(termWectorForInputFile,0);
	 	map<int,int> knnOccurrence;
	 	int result = nearestNegighbour(5,knnOccurrence);
	 	knnResult.push_back(result);
	...
	void postProcessing(){
		...
		realResult.push_back(2);
		realResult.push_back(3);
		realResult.push_back(4);
		realResult.push_back(5);
		realResult.push_back(1);
		realResult.push_back(2);
		realResult.push_back(3);
		realResult.push_back(4);
		...
		for(int i = 0 ; i < 10 ; i++){
			cout << "Real result : " << realResult[i] << "   knnRsult : " << knnResult[i] << endl;
		}
		...
		for(int i = 0 ; i < 10 ; i ++){
			...
			matrixArr[realResult[i]-1][knnResult[i]-1]++;
			
		}

		cout << "\n\n---------------------Conflusion Matrix------------------" << endl;
		for(int i = 0 ; i < 5 ; i ++){
			for(int j = 0 ;  j < 5 ; j++){
				 cout << matrixArr[i][j]  << " ";
			}
		...
		double recalltemp;
		double precisiontemp;
		double truePoisitive = 0;
		vector<double> allRecall;
		vector<double> allPrecision;
		vector<double> f1Scores;
		double f1ScorePay;
		for(int i = 0 ; i < 5 ; i++){
				truePoisitive += matrixArr[i][i]; // çarğazdakiler TP
				double reCall = 0;
				double precision = 0;
				for(int j = 0 ; j < 5 ; j++){
					reCall += matrixArr[i][j];	// satır içidekiler FN
					precision += matrixArr[j][i]; // sutun içindekiler FP
				}
				if(reCall == 0){
					recalltemp = 1;
				}else{
					recalltemp = matrixArr[i][i] / reCall;
				}
				if(precision == 0){
					precisiontemp = 1;
				}else{
					precisiontemp = matrixArr[i][i] / precision;
				}
				allRecall.push_back(recalltemp); // her label için
				allPrecision.push_back(precisiontemp); // her label için
				f1ScorePay = precisiontemp + recalltemp;

				if (f1ScorePay == 0){
					//cout << "---------> " << recalltemp << "  --------> " << precisiontemp << endl;
					f1Scores.push_back(2.0*(recalltemp*precisiontemp));
				}else{
					//cout << "---------> " << recalltemp << "  --------> " << precisiontemp <<  "--------- > " << f1ScorePay << endl;
					f1Scores.push_back(2.0*(recalltemp*precisiontemp) / f1ScorePay);
				}

		}
		double toplamDogruluk = truePoisitive / 5;
		...
		for (int i = 0 ; i < 5 ; i++){
			...
			cout << " Recall : " << allRecall[i] << "  Precision : " << allPrecision[i] << " f1Score : " << f1Scores[i] << endl;
		}
	...
	/*____________________________________________________________________________________________*/
	void printNearest(const int k, map<int, int> knnOccurrence){
		char classifications[5][10] = {"Ekonomi", "Sağlık", "Siyasi", "Spor", "Magazin"};
		for (map<int, int>::iterator it = knnOccurrence.begin(); it!= knnOccurrence.end(); ++it) 
